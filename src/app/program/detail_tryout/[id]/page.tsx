...
export default function DetailTryout() {
  ...
  const fetchTryouts = async () => {
    ...
    try {
      ...
      const { data, error: fetchError } = await supabase
        .from("tryouts")
        .select("id, name, is_active, program_id")
  ...
  const handleTryoutClick = (tryoutId: number) => {
    router.push(`/program/tryout/${tryoutId}`);
  };
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {tryouts.map((tryout) => (
          <div
            ...
            className="cursor-pointer p-6 bg-white rounded-xl border border-stone-300 shadow-md hover:shadow-xl transition-all duration-300 transform hover:scale-105"
          >
            <h3 className="text-xl font-semibold text-indigo-800 mb-2">{tryout.name}</h3>
            <p className="text-gray-600">Status: {tryout.is_active ? "Aktif" : "Tidak Aktif"}</p>
          </div>
        ))}
      </div>
    </div>
