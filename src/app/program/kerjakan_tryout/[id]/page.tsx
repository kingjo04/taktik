...
export default function KerjakanTryout() {
  ...
  const [answers, setAnswers] = useState<string[]>(new Array(10).fill("")); // Array jawaban (A, B, C, D)
  ...
  const handleAnswerSelect = (answer: string) => {
    ...
    // Map jawaban ke A, B, C, D berdasarkan opsi yang dipilih
    ...
    newAnswers[currentQuestion] = selectedIndex >= 0 ? optionKeys[selectedIndex] : "";
  ...
  const handleSubmit = async () => {
    try {
      ...
      const validAnswers = questions.map((q, index) => ({
        user_id: userId,
        tryout_id: tryoutId,
        question_id: q.id,
        user_answer: answers[index] || null, // Simpan 'A', 'B', 'C', 'D' atau null jika kosong
      ...
      // Hitung skor berdasarkan semua soal
      let correct = 0, wrong = 0, empty = 0;
      questions.forEach((q, index) => {
        const userAnswer = answers[index];
        if (userAnswer === q.correct_answer) correct++;
        else if (userAnswer) wrong++; // Hanya hitung sebagai salah jika ada jawaban
        else empty++; // Hitung sebagai kosong jika tidak ada jawaban
      });
      ...
      Swal.fire({
        icon: "success",
        title: "Selesai!",
        html: `Kamu telah menyelesaikan tryout (Attempt ${nextAttempt}).<br>
  ...
  const currentQuestionData = questions[currentQuestion];
  const selectedAnswer = answers[currentQuestion]; // Pastikan ini selalu terbaru
      ...
      <main className="flex-1 mx-auto max-w-4xl w-full">
        ...
        <div className="bg-white p-3 sm:p-4 md:p-6 rounded-xl shadow-lg mb-4 sm:mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
            <button
              key="A"
              onClick={() => handleAnswerSelect(currentQuestionData.option_a)}
              className={`p-2 sm:p-3 rounded-lg border-2 transition-all duration-300 flex items-center ${
                selectedAnswer === "A" ? "bg-blue-600 text-white border-blue-600" : "bg-gray-100 border-gray-300 hover:bg-blue-100"
              } text-sm sm:text-base`}
            >
              <span className="mr-2 text-lg font-bold">A)</span> {currentQuestionData.option_a}
            </button>
            <button
              key="B"
              onClick={() => handleAnswerSelect(currentQuestionData.option_b)}
              className={`p-2 sm:p-3 rounded-lg border-2 transition-all duration-300 flex items-center ${
                selectedAnswer === "B" ? "bg-blue-600 text-white border-blue-600" : "bg-gray-100 border-gray-300 hover:bg-blue-100"
              } text-sm sm:text-base`}
            >
              <span className="mr-2 text-lg font-bold">B)</span> {currentQuestionData.option_b}
            </button>
            <button
              key="C"
              onClick={() => handleAnswerSelect(currentQuestionData.option_c)}
              className={`p-2 sm:p-3 rounded-lg border-2 transition-all duration-300 flex items-center ${
                selectedAnswer === "C" ? "bg-blue-600 text-white border-blue-600" : "bg-gray-100 border-gray-300 hover:bg-blue-100"
              } text-sm sm:text-base`}
            >
              <span className="mr-2 text-lg font-bold">C)</span> {currentQuestionData.option_c}
            </button>
            <button
              key="D"
              onClick={() => handleAnswerSelect(currentQuestionData.option_d)}
              className={`p-2 sm:p-3 rounded-lg border-2 transition-all duration-300 flex items-center ${
                selectedAnswer === "D" ? "bg-blue-600 text-white border-blue-600" : "bg-gray-100 border-gray-300 hover:bg-blue-100"
              } text-sm sm:text-base`}
            >
              <span className="mr-2 text-lg font-bold">D)</span> {currentQuestionData.option_d}
            </button>
          </div>
        </div>
