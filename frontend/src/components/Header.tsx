...
// import { jwtDecode } from "jwt-decode"; // Komentari jika tidak dibutuhkan
// import Swal from "sweetalert2"; // Komentari jika tidak dibutuhkan

const Header = () => {
  const router = useRouter();

  const [name, setName] = useState("Guest"); // Default name untuk demo
  const [photoProfile, setPhotoProfile] = useState("");
  const [email, setEmail] = useState("guest@example.com"); // Default email untuk demo

  useEffect(() => {
    // Nonaktifkan logika autentikasi untuk demo
  ...
  const handleLogout = () => {
    // Clear the token from local storage (opsional untuk demo)
